...
export default withRouter(class Footer extends React.Component{
  ...
  componentDidMount() {
    const isHide = window && window.location && window.location.pathname.includes("admin")
    this.setState({ isHide });
  }

  componentDidUpdate(prevProps, prevState) {
    if(prevProps.router.pathname !== this.props.router.pathname) {
      const isHide = this.props.router.pathname.includes("admin");
      this.setState({ isHide });
    }
  }
  ...
  render() {
    const { categories, newestList } = this.props;
